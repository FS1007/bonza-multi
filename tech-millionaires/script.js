...
const CELL_SIZE = 50;        // Size of each grid cell (50x50 pixels)
...
function setupEventListeners() {
    ...
    // Drag and drop events for all pieces
...
function arePiecesConnected(wordPieces, wordData) {
    ...
    // Check if pieces are actually connected (touching) with STRICT alignment
    for (let i = 0; i < sortedPieces.length - 1; i++) {
        ...
        if (orientation === 'horizontal') {
            // For horizontal: pieces must be touching AND perfectly aligned vertically
            const gap = Math.abs(nextRect.left - currentRect.right);
            const verticalAlign = Math.abs(currentRect.top - nextRect.top);
            
            // STRICT: Gap must be 0-2px (touching), vertical alignment must be perfect (0-2px)
            if (gap > 2 || verticalAlign > 2) {
                console.log(`  ✗ Horizontal alignment failed: gap=${gap.toFixed(1)}px, vAlign=${verticalAlign.toFixed(1)}px`);
                return false;
            }
        } else {
            // For vertical: pieces must be touching AND perfectly aligned horizontally
            const gap = Math.abs(nextRect.top - currentRect.bottom);
            const horizontalAlign = Math.abs(currentRect.left - nextRect.left);
            
            // STRICT: Gap must be 0-2px (touching), horizontal alignment must be perfect (0-2px)
            if (gap > 2 || horizontalAlign > 2) {
...
console.log('  - CELL_SIZE:', CELL_SIZE + 'px');
console.log('  - SNAP_DISTANCE:', SNAP_DISTANCE + 'px');
